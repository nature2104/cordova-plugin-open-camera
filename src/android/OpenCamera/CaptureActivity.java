...
import com.cordova.mediacapture.OpenCamera.CameraController.CameraControllerManager2;
...
import com.cordova.mediacapture.OpenCamera.UI.MainUI;
 ...
 */
public class CaptureActivity extends Activity implements AudioListener.AudioListenerCallback {
    ...
    private final Map<Integer, Bitmap> preloaded_bitmap_resources = new Hashtable<Integer, Bitmap>();
...
//		cordova.getActivity().getString(cordova.getActivity().getResources().getIdentifier( "activity_capture_media", "layout", cordova.getActivity().getPackageName()));
		setContentView(getResources().getIdentifier("activity_capture_media", "layout", getPackageName()));
		PreferenceManager.setDefaultValues(this, getResources().getIdentifier("oc_preferences", "xml", getPackageName()), false); // initialise any unset preferences to their default values
		if( MyDebug.LOG )
	    ...
	    View switchCameraButton = findViewById(getResources().getIdentifier("switch_camera", "id", getPackageName()));
	    switchCameraButton.setVisibility(preview.getCameraControllerManager().getNumberOfCameras() > 1 ? View.VISIBLE : View.GONE);
	    View speechRecognizerButton = findViewById(getResources().getIdentifier("audio_control", "id", getPackageName()));
		...
		View pauseVideoButton = findViewById(getResources().getIdentifier("pause_video", "id", getPackageName()));
		...
		// however).
	    View takePhotoButton = findViewById(getResources().getIdentifier("take_photo", "id", getPackageName()));
		takePhotoButton.setVisibility(View.INVISIBLE);
	    View zoomControls = findViewById(getResources().getIdentifier("zoom", "id", getPackageName()));
		zoomControls.setVisibility(View.INVISIBLE);
	    View zoomSeekbar = findViewById(getResources().getIdentifier("zoom_seekbar", "id", getPackageName()));
	    ...
	    orientationEventListener = new OrientationEventListener(this) {
			...
			public void onOrientationChanged(int orientation) {
				CaptureActivity.this.mainUI.onOrientationChanged(orientation);
			}
        };
		if( MyDebug.LOG )
        ...
        View galleryButton = findViewById(getResources().getIdentifier("gallery", "id", getPackageName()));
        ...
        // load icons
        preloadIcons(getResources().getIdentifier("flash_icons", "array", getPackageName()));
        preloadIcons(getResources().getIdentifier("focus_mode_icons", "array", getPackageName()));
		if( MyDebug.LOG )
        ...
        new Thread(new Runnable() {
            public void run() {
                textToSpeech = new TextToSpeech(CaptureActivity.this, new TextToSpeech.OnInitListener() {
        			@Override
	...
	public void zoomIn() {
		mainUI.changeSeekbar(getResources().getIdentifier("zoom_seekbar", "id", getPackageName()), -1);
	}
	...
	public void changeExposure(int change) {
		mainUI.changeSeekbar(getResources().getIdentifier("exposure_seekbar", "id", getPackageName()), change);
	}

	public void changeISO(int change) {
		mainUI.changeSeekbar(getResources().getIdentifier("iso_seekbar", "id", getPackageName()), change);
	}

	public void changeFocusDistance(int change) {
		mainUI.changeSeekbar(getResources().getIdentifier("focus_seekbar", "id", getPackageName()), change);
	}
    ...
    protected void onResume() {
        ...
        initLocation();
        initSound();
    	loadSound(getResources().getIdentifier("beep", "raw", getPackageName()));
    	loadSound(getResources().getIdentifier("beep_hi", "raw", getPackageName()));
    ...
    public void clickedAudioControl(View view) {
        ...
        if( audio_control.equals("voice") && speechRecognizer != null ) {
        	if( speechRecognizerIsStarted ) {
		    	...
		    	preview.showToast(audio_control_toast, getResources().getIdentifier("speech_recognizer_started", "string", getPackageName()));
        	...
        	if( audio_listener != null ) {
		    	...
		    	preview.showToast(audio_control_toast, getResources().getIdentifier("audio_listener_started", "string", getPackageName()));
        		startAudioListener();
        	}
        }
    }
	...
	private void setProgressSeekbarScaled(SeekBar seekBar, double min_value, double max_value, double value) {
		...
		double frac = CaptureActivity.seekbarScalingInverse(scaling);
    ...
    public void clickedExposureLock(View view) {
		...
		preview.showToast(exposure_lock_toast, preview.isExposureLocked() ? getResources().getIdentifier("exposure_locked", "string", getPackageName()) : getResources().getIdentifier("exposure_unlocked", "string", getPackageName()));
    }
	...
	class PreferencesListener implements SharedPreferences.OnSharedPreferenceChangeListener {
		...
		public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
			...
			List<String> keys = new ArrayList<String>() {{
				add( "preference_timer");
				add( "preference_touch_capture");
				add( "preference_pause_preview");
				add( "preference_shutter_sound");
				add( "preference_timer_beep");
				add( "preference_timer_speak");
				add( "preference_volume_keys");
				add( "preference_audio_noise_control_sensitivity");
				add( "preference_using_saf");
				add( "preference_save_photo_prefix");
				add( "preference_save_video_prefix");
				add( "preference_save_zulu_time");
				add( "preference_show_when_locked");
				add( "preference_startup_focus");
				add( "preference_show_zoom");
				add( "preference_show_angle");
				add( "preference_show_angle_line");
				add( "preference_show_pitch_lines");
				add( "preference_angle_highlight_color");
				add( "preference_show_geo_direction");
				add( "preference_show_geo_direction_lines");
				add( "preference_show_battery");
				add( "preference_show_time");
				add( "preference_free_memory");
				add( "preference_show_iso");
				add( "preference_grid");
				add( "preference_crop_guide");
				add( "preference_show_toasts");
				add( "preference_thumbnail_animation");
				add( "preference_take_photo_border");
				add( "preference_keep_display_on");
				add( "preference_max_brightness");
				add( "preference_hdr_save_expo");
				add( "preference_front_camera_mirror");
				add( "preference_stamp");
				add( "preference_stamp_dateformat");
				add( "preference_stamp_timeformat");
				add( "preference_stamp_gpsformat");
				add( "preference_textstamp");
				add( "preference_stamp_fontsize");
				add( "preference_stamp_font_color");
				add( "preference_stamp_style");
				add( "preference_background_photo_saving");
				add( "preference_record_audio");
				add( "preference_record_audio_src");
				add( "preference_record_audio_channels");
				add( "preference_lock_video");
				add( "preference_video_subtitle");
				add( "preference_require_location");
			}};
			Boolean isExist = false;
			for( String defKey : keys){
    ...
    public void openSettings() {
        ...
        getFragmentManager().beginTransaction().add(getResources().getIdentifier("prefs_container", "id", getPackageName()), fragment, "PREFERENCE_FRAGMENT").addToBackStack(null).commitAllowingStateLoss();
    }
    ...
    public void onBackPressed() {
        ...
        if( screen_is_locked ) {
			preview.showToast(screen_locked_toast, getResources().getIdentifier("screen_is_locked", "string", getPackageName()));
        	return;
        }
        if( fragment != null ) {
			if( MyDebug.LOG )
    ...
    public void showPreview(boolean show) {
		...
		final ViewGroup container = (ViewGroup)findViewById(getResources().getIdentifier("hide_container", "id", getPackageName()));
	...
	private void updateGalleryIconToBlank() {
    	...
    	ImageButton galleryButton = (ImageButton) this.findViewById(getResources().getIdentifier("gallery", "id", getPackageName()));
		...
		galleryButton.setImageResource(getResources().getIdentifier("gallery", "drawable", getPackageName()));
    ...
    public void updateGalleryIcon() {
		...
		new AsyncTask<Void, Void, Bitmap>() {
			private static final String TAG = "Capture/AsyncTask";
		    ...
		    protected Bitmap doInBackground(Void... params) {
				...
				KeyguardManager keyguard_manager = (KeyguardManager)CaptureActivity.this.getSystemService(Context.KEYGUARD_SERVICE);
	...
	void savingImage(final boolean started) {
		...
		this.runOnUiThread(new Runnable() {
			public void run() {
				final ImageButton galleryButton = (ImageButton) findViewById(getResources().getIdentifier("gallery", "id", getPackageName()));
				if( started ) {
    ...
    public void clickedGallery(View view) {
        ...
        try {
            ...
            if( intent.resolveActivity(getPackageManager()) != null ) {
                ...
                preview.showToast(null, getResources().getIdentifier("no_gallery_app", "string", getPackageName()));
            }
        }
    }
        ...
        if( requestCode == 42 ) {
            if( resultCode == RESULT_OK && resultData != null ) {
				...
				try {
					...
					if( file != null ) {
						preview.showToast(null, getResources().getString(getResources().getIdentifier("changed_save_location", "string", getPackageName())) + "\n" + file.getAbsolutePath());
					}
				}
					...
					if( uri.length() == 0 ) {
						...
						editor.apply();
						preview.showToast(null, getResources().getIdentifier("saf_permission_failed", "string", getPackageName()));
					}
				}
	        }
	        else {
	    		...
	    		if( uri.length() == 0 ) {
	    			...
	    			editor.putBoolean(PreferenceKeys.getUsingSAFPreferenceKey(), false);
	    			editor.apply();
	    			preview.showToast(null, getResources().getIdentifier("saf_cancelled", "string", getPackageName()));
	    		}
	        }
	...
	void updateSaveFolder(String new_save_location) {
		...
		if( new_save_location != null ) {
			...
			if( !orig_save_location.equals(new_save_location) ) {
				...
				this.preview.showToast(null, getResources().getString(getResources().getIdentifier("changed_save_location", "string", getPackageName())) + "\n" + this.applicationInterface.getStorageUtils().getSaveLocation());
			}
		}
	}
    ...
    private void longClickedGallery() {
        ...
        alertDialog.setTitle(getResources().getIdentifier("choose_save_location", "string", getPackageName()));
        ...
        items[index++] = getResources().getString(getResources().getIdentifier("clear_folder_history", "string", getPackageName()));
        final int new_index = index;
        items[index++] = getResources().getString(getResources().getIdentifier("choose_another_folder", "string", getPackageName()));
			...
			public void onClick(DialogInterface dialog, int which) {
				if( which == clear_index ) {
				    ...
				    new AlertDialog.Builder(CaptureActivity.this)
			        	...
			        	.setTitle(getResources().getIdentifier("clear_folder_history", "string", getPackageName()))
			        	.setMessage(getResources().getIdentifier("clear_folder_history_question", "string", getPackageName()))
			        	.setPositiveButton(getResources().getIdentifier("answer_yes", "string", getPackageName()), new DialogInterface.OnClickListener() {
			        		@Override
					        public void onClick(DialogInterface dialog, int which) {
								if( MyDebug.LOG )
			        	...
			        	.setNegativeButton(getResources().getIdentifier("answer_no", "string", getPackageName()), new DialogInterface.OnClickListener() {
			        		@Override
					        public void onClick(DialogInterface dialog, int which) {
								if( MyDebug.LOG )
     ...
     */
    void lockScreen() {
		findViewById(getResources().getIdentifier("locker", "id", getPackageName())).setOnTouchListener(new View.OnTouchListener() {
     ...
     */
    void unlockScreen() {
		findViewById(getResources().getIdentifier("locker", "id", getPackageName())).setOnTouchListener(null);
	...
	private class MyGestureDetector extends SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            try {
        		...
        		final ViewConfiguration vc = ViewConfiguration.get(CaptureActivity.this);
                ...
                if( dist2 > swipeMinDistance*swipeMinDistance && vel2 > swipeThresholdVelocity*swipeThresholdVelocity ) {
                	preview.showToast(screen_locked_toast, getResources().getIdentifier("unlocked", "string", getPackageName()));
                	unlockScreen();
                }
            }
            catch(Exception e) {
    ...
    void cameraSetup() {
    	...
    	SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		{
		    ...
		    ZoomControls zoomControls = (ZoomControls) findViewById(getResources().getIdentifier("zoom", "id", getPackageName()));
		    SeekBar zoomSeekBar = (SeekBar) findViewById(getResources().getIdentifier("zoom_seekbar", "id", getPackageName()));
		...
		}
		{
		    ...
		    SeekBar focusSeekBar = (SeekBar) findViewById(getResources().getIdentifier("focus_seekbar", "id", getPackageName()));
		    ...
		    focusSeekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
				...
				public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
					...
					double scaling = CaptureActivity.seekbarScaling(frac);
			...
			Log.d(TAG, "cameraSetup: time after setting up manual focus: " + (System.currentTimeMillis() - debug_time));
		{
			if( preview.supportsISORange()) {
				...
				SeekBar iso_seek_bar = ((SeekBar)findViewById(getResources().getIdentifier("iso_seekbar", "id", getPackageName())));
				...
				if( preview.supportsExposureTime() ) {
					...
					SeekBar exposure_time_seek_bar = ((SeekBar)findViewById(getResources().getIdentifier("exposure_time_seekbar", "id", getPackageName())));
			...
			Log.d(TAG, "cameraSetup: time after setting up iso: " + (System.currentTimeMillis() - debug_time));
		{
			if( preview.supportsExposures() ) {
				...
				SeekBar exposure_seek_bar = ((SeekBar)findViewById(getResources().getIdentifier("exposure_seekbar", "id", getPackageName())));
				...
				ZoomControls seek_bar_zoom = (ZoomControls)findViewById(getResources().getIdentifier("exposure_seekbar_zoom", "id", getPackageName()));
    ...
    public void setManualWBSeekbar() {
		...
		if( preview.getSupportedWhiteBalances() != null && preview.supportsWhiteBalanceTemperature() ) {
			...
			SeekBar white_balance_seek_bar = ((SeekBar)findViewById(getResources().getIdentifier("white_balance_seekbar", "id", getPackageName())));
	...
	private void showPhotoVideoToast(boolean always_show) {
		...
		if( preview.isVideo() ) {
			...
			toast_string = getResources().getString(getResources().getIdentifier("video", "string", getPackageName())) + ": " + profile.videoFrameWidth + "x" + profile.videoFrameHeight + ", " + profile.videoFrameRate + "fps, " + bitrate_string;
			boolean record_audio = sharedPreferences.getBoolean(PreferenceKeys.getRecordAudioPreferenceKey(), true);
			if( !record_audio ) {
				toast_string += "\n" + getResources().getString(getResources().getIdentifier("audio_disabled", "string", getPackageName()));
				simple = false;
			}
			...
			if( max_duration_value.length() > 0 && !max_duration_value.equals("0") ) {
				String [] entries_array = getResources().getStringArray(getResources().getIdentifier("preference_video_max_duration_entries", "array", getPackageName()));
				...
				if( index != -1 ) { // just in case!
					...
					toast_string += "\n" + getResources().getString(getResources().getIdentifier("max_duration", "string", getPackageName())) +": " + entry;
					simple = false;
				}
			}
			...
			if( max_filesize != 0 ) {
				...
				toast_string += "\n" + getResources().getString(getResources().getIdentifier("max_filesize", "string", getPackageName())) +": " + max_filesize_mb + getResources().getString(getResources().getIdentifier("mb_abbreviation", "string", getPackageName()));
				simple = false;
			}
			if( sharedPreferences.getBoolean(PreferenceKeys.getVideoFlashPreferenceKey(), false) && preview.supportsFlash() ) {
				toast_string += "\n" + getResources().getString(getResources().getIdentifier("preference_video_flash", "string", getPackageName()));
				simple = false;
			}
		}
		else {
			toast_string = getResources().getString(getResources().getIdentifier("photo", "string", getPackageName()));
			...
			if( sharedPreferences.getBoolean(PreferenceKeys.getAutoStabilisePreferenceKey(), false) ) {
				...
				toast_string += "\n" + getResources().getString(getResources().getIdentifier("preference_auto_stabilise", "string", getPackageName()));
				simple = false;
			}
			...
			if( photo_mode == MyApplicationInterface.PhotoMode.DRO ) {
				photo_mode_string = getResources().getString(getResources().getIdentifier("photo_mode_dro", "string", getPackageName()));
			}
			else if( photo_mode == MyApplicationInterface.PhotoMode.HDR ) {
				photo_mode_string = getResources().getString(getResources().getIdentifier("photo_mode_hdr", "string", getPackageName()));
			}
			else if( photo_mode == MyApplicationInterface.PhotoMode.ExpoBracketing ) {
				photo_mode_string = getResources().getString(getResources().getIdentifier("photo_mode_expo_bracketing_full", "string", getPackageName()));
			}
			if( photo_mode_string != null ) {
				toast_string += "\n" + getResources().getString(getResources().getIdentifier("photo_mode", "string", getPackageName())) + ": " + photo_mode_string;
				simple = false;
			}
		}
    	...
    	if( scene_mode != null && !scene_mode.equals(camera_controller.getDefaultSceneMode()) ) {
    		toast_string += "\n" + getResources().getString(getResources().getIdentifier("scene_mode", "string", getPackageName())) + ": " + mainUI.getEntryForSceneMode(scene_mode);
			simple = false;
    	}
    	...
    	if( white_balance != null && !white_balance.equals(camera_controller.getDefaultWhiteBalance()) ) {
    		toast_string += "\n" + getResources().getString(getResources().getIdentifier("white_balance", "string", getPackageName())) + ": " + mainUI.getEntryForWhiteBalance(white_balance);
    	...
    	if( color_effect != null && !color_effect.equals(camera_controller.getDefaultColorEffect()) ) {
    		toast_string += "\n" + getResources().getString(getResources().getIdentifier("color_effect", "string", getPackageName())) + ": " + mainUI.getEntryForColorEffect(color_effect);
			simple = false;
    	}
		...
		if( !lock_orientation.equals("none") ) {
			String [] entries_array = getResources().getStringArray(getResources().getIdentifier("preference_lock_orientation_entries", "array", getPackageName()));
			String [] values_array = getResources().getStringArray(getResources().getIdentifier("preference_lock_orientation_values", "array", getPackageName()));
		...
		if( !timer.equals("0") ) {
			String [] entries_array = getResources().getStringArray(getResources().getIdentifier("preference_timer_entries", "array", getPackageName()));
			String [] values_array = getResources().getStringArray(getResources().getIdentifier("preference_timer_values", "array", getPackageName()));
			int index = Arrays.asList(values_array).indexOf(timer);
			if( index != -1 ) { // just in case!
				String entry = entries_array[index];
				toast_string += "\n" + getResources().getString(getResources().getIdentifier("preference_timer", "string", getPackageName())) + ": " + entry;
				simple = false;
			}
		}
		String repeat = applicationInterface.getRepeatPref();
		if( !repeat.equals("1") ) {
			String [] entries_array = getResources().getStringArray(getResources().getIdentifier("preference_burst_mode_entries", "array", getPackageName()));
			String [] values_array = getResources().getStringArray(getResources().getIdentifier("preference_burst_mode_values", "array", getPackageName()));
			int index = Arrays.asList(values_array).indexOf(repeat);
			if( index != -1 ) { // just in case!
				String entry = entries_array[index];
				toast_string += "\n" + getResources().getString(getResources().getIdentifier("preference_burst_mode", "string", getPackageName())) + ": " + entry;
				simple = false;
			}
		}
		/*if( audio_listener != null ) {
			toast_string += "\n" + getResources().getString(getResources().getIdentifier("preference_audio_noise_control", "string", getPackageName()));
		}*/
	...
	private void startAudioListener() {
		...
		if( audio_listener.status() ) {
			...
			if(sensitivity_pref == "3")
				audio_noise_sensitivity = 50;
			else if(sensitivity_pref == "2")
				audio_noise_sensitivity = 75;
			else if(sensitivity_pref == "1")
			...
			audio_listener = null;
			preview.showToast(null, getResources().getIdentifier("audio_listener_failed", "string", getPackageName()));
		}
	}
	...
	private void showRequestPermissionRationale(final int permission_code) {
		...
		if( permission_code == MY_PERMISSIONS_REQUEST_CAMERA ) {
			...
			message_id = getResources().getIdentifier("permission_rationale_camera", "string", getPackageName());
		}
		else if( permission_code == MY_PERMISSIONS_REQUEST_STORAGE ) {
			if( MyDebug.LOG )
				Log.d(TAG, "display rationale for storage permission");
			permissions = new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE};
			message_id = getResources().getIdentifier("permission_rationale_storage", "string", getPackageName());
		}
			...
			message_id = getResources().getIdentifier("permission_rationale_record_audio", "string", getPackageName());
		}
			...
			message_id = getResources().getIdentifier("permission_rationale_location", "string", getPackageName());
		}
		else {
			if( MyDebug.LOG )
		...
		if( ok ) {
			...
			.setTitle(getResources().getIdentifier("permission_rationale_title", "string", getPackageName()))
			...
			.setOnDismissListener(new OnDismissListener() {
				public void onDismiss(DialogInterface dialog) {
					...
					ActivityCompat.requestPermissions(CaptureActivity.this, permissions_f, permission_code);
				}
			}).show();
		}
	}
		...
		switch( requestCode ) {
	        ...
	        case MY_PERMISSIONS_REQUEST_LOCATION:
	        {
	            ...
	            if( grantResults.length > 0
		    		...
		    		preview.showToast(null, getResources().getIdentifier("permission_location_not_available", "string", getPackageName()));
